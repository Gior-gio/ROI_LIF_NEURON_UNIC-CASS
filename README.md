...
The goal is to implement the three core building blocks of the LIF neuron: the integrator, the threshold circuit, and the spike generator. By wiring these up with one external resistor, the neuron’s behavior can be observed when input voltage pulses are applied. The design also includes a controlled reset through the Vrst pin. Depending on pin availability, extra outputs will be added to monitor the Vmem integrator output and the phases of the two ring oscillators.
